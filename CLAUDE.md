...
│       ├── hooks/                # useWeatherData, useTransitData, useSubwayArrivals, useSubwayAlerts, useBusArrivals, useCardConfig, useLocalStorage, useMockData
│       ├── context/              # DashboardContext (Context API 기반 상태관리)
│       ├── data/                 # cardRegistry, mock 데이터
│       └── utils/                # cn, time
└── src/main/java/com/ready4work/
    ├── config/                   # WebClientConfig (7개 Bean), DotenvConfig
    ├── weather/                  # Controller / Service / ClothingAdvisor / dto
    ├── transit/                  # Controller / Service / EnrichmentService / dto / util
    ├── bus/                      # BusArrivalController / BusArrivalService / dto
    └── subway/                   # ArrivalService / AlertService / Controller / dto
```

### 레이어 규칙
- **도메인 분리**: `weather`, `transit`, `subway` 각자 Controller/Service/dto 보유. 도메인 간 직접 의존 금지 (공통은 `config/`). 예외: `TransitEnrichmentService`는 조합 계층으로서 `subway`, `bus` 서비스를 주입받아 실시간 데이터를 통합.
...
- **캐싱**: `ConcurrentHashMap<String, Mono<X>> + Mono.cache(Duration)` 패턴. TTL — Weather 30분, Transit 경로 5분·좌표 24h, Subway Arrival 15초, Alert 1분, Bus 좌표→stId 24h·도착 15초.
...
| GET | `/api/transit/routes?origin&destination&refresh&enrich` | `TransitService` + `TransitEnrichmentService` | 30분 (enrich 시 실시간 15초) |
| GET | `/api/subway/arrivals?stationName&refresh` | `SubwayArrivalService` | 15초 |
| GET | `/api/alerts/subway?refresh` | `SubwayAlertService` | 1분 |
| GET | `/api/bus/arrivals?lat&lon&refresh` | `BusArrivalService` | 좌표→stId 24h, 도착 15초 |
...
- `ODSAY_API_KEY` — ODsay LAB 서버 키
- `SEOUL_DATA_API_KEY` — 서울 열린데이터광장
- `DATA_GO_KR_SERVICE_KEY` — 공공데이터포털 (URL-encoded 상태로 입력)
- `DATA_GO_KR_BUS_STATION_INFO_KEY` — 공공데이터포털 버스 정류소정보 (URL-encoded)
- `DATA_GO_KR_BUS_ARRIVAL_INFO_KEY` — 공공데이터포털 버스 도착정보 (URL-encoded)
